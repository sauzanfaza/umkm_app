...
export default function PenjualanHarian() {
    ...
    // Summary
    const totalStokAwal = data.reduce((sum, item) => sum + (Number(item.stok_awal) || 0), 0)
    const totalStokAkhir = data.reduce((sum, item) => sum + (Number(item.stok_akhir) || 0), 0)
    ...
    // Cek semua stok akhir sudah diisi
    const semuaStokAkhirTerisi = data.length > 0 &&
        data.every(item => item.stok_akhir !== null && item.stok_akhir !== undefined && item.stok_akhir !== "")
                ...
                <div className="py-6 px-8 text-lg">
                    ...
                    {data.length > 0 && !semuaStokAkhirTerisi && (
                        <p className="text-sm text-amber-500 mt-2 text-right">
                            * Isi semua stok akhir terlebih dahulu untuk closing
                        </p>
                    )}
                </div>
            </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 w-[480px] shadow-2xl">
                        ...
                        <div className="bg-slate-50 rounded-xl p-4 mb-4 space-y-2 text-sm">
                            ...
                            <div className="flex justify-between">
                                <span className="text-slate-500">Total Stok Awal</span>
                                <span className="font-medium">{totalStokAwal} pcs</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">Total Stok Akhir (Sisa)</span>
                                <span className="font-medium">{totalStokAkhir} pcs</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-slate-500">Total Terjual</span>
                                <span className="font-medium">{totalTerjual} pcs</span>
                            </div>
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                className="flex-1 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 font-medium"
                            >
                                Batal
                            </button>
                            <button
